...
for idx, row in data.iterrows():
    if pd.isna(data.at[idx, "home_pitcher"]):
        if not np.isnan(row["home_pitcher_rest"]):
            days = int(row["home_pitcher_rest"])
        else:
            continue
        ...
        if not candidates.empty:
            ...
            if not candidates["home_pitcher"].isna().all():
                data.at[idx, "home_pitcher"] = (
                    candidates["home_pitcher"].dropna().iloc[0]
                )
            elif not candidates["away_pitcher"].isna().all():
                data.at[idx, "home_pitcher"] = (
                    candidates["away_pitcher"].dropna().iloc[0]
                )


for idx, row in data.iterrows():
    if pd.isna(data.at[idx, "away_pitcher"]):
        if not np.isnan(row["away_pitcher_rest"]):
            days = int(row["away_pitcher_rest"])
        else:
            continue
...
data.to_csv("preprocess_2.csv", index=False)
